  ...
  return (
    <div className="space-y-6">
      <GlassPanel className="p-6">
        <h1 className="text-xl font-semibold mb-1">📞 Self-Talk Progressions</h1>
        <p className="text-xs text-[var(--text-secondary)] mb-4">
          每次你腦中自然冒出日文，撳一吓 log 低。第一年目標：由 Stage 1 → 4。
        </p>
        <div className="flex items-center gap-4 mb-2">
          <div className="text-[10px] uppercase tracking-[0.2em] text-[var(--text-muted)]">Today max stage</div>
          <div className="text-2xl font-semibold text-[var(--accent-lime)]">
            {todayMaxStage === 0 ? "—" : `Stage ${todayMaxStage}`}
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2 mt-3">
          {[1, 2, 3, 4].map((s) => (
            <div key={s} className={`p-3 rounded-lg border ${todayMaxStage >= s ? "border-[var(--accent-lime)]/40 bg-[var(--accent-lime-bg)]/30" : "border-white/10 bg-white/[0.02]"}`}>
              <div className="text-xs font-semibold">Stage {s}</div>
      ...
      <section className="space-y-2">
        <h2 className="text-sm font-semibold px-1">Recent 7 days</h2>
        {(recent ?? []).length === 0 ? (
          <GlassPanel variant="subtle" className="p-6 text-center text-sm text-[var(--text-secondary)]">
            尚未 log 過。腦中冒出一個日文字都係 Stage 1，撳上面快速 log。
          </GlassPanel>
        ) : (
            ...
            {(recent ?? []).map((r) => (
              <GlassPanel key={r.id} variant="subtle" className="p-3">
                <div className="flex items-center gap-2 text-[10px] text-[var(--text-muted)]">
                  ...
                  <span className="px-1.5 py-0.5 rounded bg-[var(--accent-sakura)]/10 text-[var(--accent-sakura)]">Stage {r.stage_level}</span>

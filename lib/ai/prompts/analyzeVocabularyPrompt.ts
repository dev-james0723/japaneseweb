...
export function buildAnalyzeVocabularyPrompt(items: AnalyzeVocabInputItem[]) {
  ...
  return [
    ...
    `每個項目需要：japanese、kana、romaji（Hepburn）、meaning_zh（繁中）、meaning_en、part_of_speech、jlpt_level（字串，如 "N5"、"N4"；勿用純數字）、priority_tier（整數 1–3：1=主動產出, 2=被動識別, 3=輕度接觸）、register_label、core_explanation。`,
